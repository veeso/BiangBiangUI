...
//    • All user-facing strings come from ctx.config.strings; the GitHub repo
//      and support email come from ctx.config.branding. The DEBUG-only
//      review-reset block is kept verbatim from the reference (debug-only,
//      developer-facing text — never shipped to end users).
//
...
public struct SettingsScreen: View {
    ...
    public var body: some View {
        ...
        NavigationStack {
            Form {
                sharedCoreSections(settings: $settings)
                variantSection(settings: $settings)
                descriptorSections()
    ...
    @ViewBuilder
    private func sharedCoreSections(settings: Bindable<SettingsStore>) -> some View {
        let translatable = ctx.activeVariant?.translatable ?? true

        Section {
            Picker(
                ctx.config.strings.translationLanguage,
                selection: settings.userLanguage
            ) {
    ...
    @ViewBuilder
    private func variantSection(settings: Bindable<SettingsStore>) -> some View {
        let variants = ctx.activeProfile.variants
        if variants.count > 1 {
            Section {
                Picker(
                    ctx.activeProfile.displayName,
                    selection: settings.selectedVariantId
                ) {
